...
VALUES  ( @literalId,
    ...
    public async Task SaveFileScan(int _, string filename, Dictionary<string, Location> locations)
    {
        ...
        foreach (var i in locations)
        {
            ...
            cmd.Parameters.Add(new SqlParameter("@line", i.Value.Start.Line));
            cmd.Parameters.Add(new SqlParameter("@char", i.Value.Start.Character));
            cmd.Parameters.Add(new SqlParameter("@isConstant", i.Value.IsConstant));
            await cmd.ExecuteNonQueryAsync();
        }
    }
